 ...
 * First connect creates indexes and seeds demo data.
 */
...
const toRow = <T extends { id: string }>({ id, ...rest }: T): Row<T> => ({
	_id: id,
	...rest
});
...
async function connect(uri: string, dbName: string): Promise<Db> {
	...
	/*
	 * USERS
	 * Only insert seeded users that don't already exist.
	 * Existing / real users are never overwritten.
	 */
	...
	if (missingUsers.length) {
		await db
			.collection<UserRow>('users')
			.insertMany(missingUsers.map(toRow));

		console.log(`[db] seeded ${missingUsers.length} demo users`);
	}

	/*
	 * ACTIVITIES
	 * Same logic: add newly-created seed activities without deleting or
	 * overwriting anything already in Mongo.
	 */
	...
	const existingActivityIds = new Set(
		existingActivities.map((a) => a._id)
	);
	...
	if (missingActivities.length) {
		await db
			.collection<ActivityRow>('activities')
			.insertMany(missingActivities.map(toRow));

		console.log(
			`[db] seeded ${missingActivities.length} new activities`
		);
	}

	/*
	 * COMMENTS
	 * Keep your original behavior for comments.
	 */
	if (
		(await db
		...
		await db
			.collection<CommentRow>('comments')
			.insertMany(seedComments.map(toRow));
...
export function createMongoStore(
	...
	const db = () =>
		(globalThis.__tagalongMongo ??= connect(uri, dbName));

	const users = async (): Promise<Collection<UserRow>> =>
		(await db()).collection('users');

	const activities = async (): Promise<Collection<ActivityRow>> =>
		(await db()).collection('activities');

	const comments = async (): Promise<Collection<CommentRow>> =>
